...
imageRouter.post("/images/posterize", async (req, resp) => {
...
imageRouter.post("/images/favorites/toggle", async (req, resp) => {
	try {
		const payload: any = req.body;
...
imageRouter.post("/images/favorites", async (req, resp) => {
	try {
		const payload: any = req.body;
		const userToken = jwtDecode(payload.token);
		let favorites = [];
